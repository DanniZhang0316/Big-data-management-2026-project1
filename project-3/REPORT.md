...
The aggregation layer groups records by entity_id and computes lifecycle-level customer metrics. The first_seen_ts value is derived from the real database created_at timestamp, stored in microseconds, rather than from the Debezium ingestion timestamp. Customer status is determined by joining against silver_customers. However, deletion history takes priority: if a customer was ever deleted, they are marked as deleted even if they still appear in silver_customers.
...
The query output for `gold_customer_activity` is available in the notebook [`gold_customer_activity.ipynb`](gold_customer_activity.ipynb).  
To keep the terminal output clean, the print statement was removed from `gold_customer_activity.py`.

![Task 3 scenario table](Task3_scenario_table.png)
